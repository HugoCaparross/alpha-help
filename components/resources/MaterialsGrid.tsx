...
export default function MaterialsGrid({ materials }: MaterialsGridProps) {
  const [selectedMaterial, setSelectedMaterial] = useState<StudyMaterialWithStatus | null>(null);
  const timeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const hasRegisteredRef = useRef(false);

  const openMaterial = useCallback((material: StudyMaterialWithStatus) => {
    if (material.status !== "available" || !material.pdfUrl) return;
    ...
    <>
      <section className="materials-grid" aria-label="Listado de materiales">
        {materials.map((material) => (
          <MaterialCard key={material.id} material={material} onOpen={openMaterial} />
        ))}
      </section>

      <Modal open={selectedMaterial !== null} title={modalTitle} onClose={closeMaterial} maxWidth={1200}>
        {selectedMaterial?.status === "available" && selectedMaterial.pdfUrl ? (
          <PdfViewer title={selectedMaterial.title} pdfUrl={selectedMaterial.pdfUrl} />
        ) : null}
      </Modal>
    </>
  );
