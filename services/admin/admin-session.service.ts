...
export async function setAdminSessionLiveEnded(id: string, ended: boolean): Promise<void> {
  const response = await fetch(`/api/admin/sessions/${id}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ liveEnded: ended }),
  });
  if (!response.ok) {
    const data = await response.json().catch(() => null);
    throw new Error(data?.error ?? "No se ha podido actualizar el estado de la sesión.");
  }
}
